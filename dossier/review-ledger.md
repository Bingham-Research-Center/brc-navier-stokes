...
| [Averaging portability barrier](experiments/averaging-portability-barrier.md) | The useful barrier is operator-relative: constants may depend on Tao's selected averaged operator.  Portability of the full q4 stack is a candidate classification pending item-by-item Lorentz transfer.  A shell realization tests only its enumerated budgets; nonrealizability does not automatically produce a portable charge.  An averaged equation may have a pressure representation but lacks the canonical true-NSE local quadratic pressure structure. |
